...
| `tests/docs/boundary-table.test.ts` | `usage-guide.md` **§7「已知边界」的 77 行**与守卫登记表一一对应：每条边界要么 `pin`（指向真钉它的用例）、要么 `choice`（设计选择，带理由）、要么 `gap`（附最可能接上的用例）；且 §7 里每个反引号 token 都指向**真实存在**的东西 | 解析 §7 表 + **双向**集合比对 + `pin={file,marker}` 逐条可证伪 + 三态下限（`pin ≥ 30` / `choice ≥ 8`）+ 语料防真空（>500k 字符、token ≥ 200、纯标识符 ≥ 100）。⚠️ `marker` 必须是那条 `it(...)` 的**整条标题**（不是子串），非用例文件（脚本）退回「逐字在场 + 长度 ≥ 12」。**反向验证过 8 条（2026-09-26 首版，各恰好点名那条）**：M1 §7 凭空加一行 ⇒ **恰好 A2**「文档有边界但没登记」；M2 删一行 ⇒ **恰好 A2**「登记了但文档里没有」；M3 `idempotencyKey`→`…ZZ` ⇒ **恰好 A1**（点名第 1399 行）；M4 被引用用例的标题加一个字 ⇒ **恰好 A3**；M5 `pin` 指向不存在的文件（两条）⇒ **恰好 A3**；M6 语料清空 ⇒ **恰好 A1** 的防真空下限；M7 `gap` 的候选不存在 ⇒ **恰好 A3b**；M8 全 38 条 `pin` 降级成 `choice` ⇒ **A3 + A4 两条**。⚠️ **第一版 A3 是假守卫**：它用 `titles.some(t => t.includes(marker))`，M4 给标题加一个字后**照样绿** —— 与本仓记过的 `rawArg`→`rawArgument`（子串匹配照样绿，M76 exit 0）**同一个坑** ⇒ 已改成整条标题相等 | §7 是使用者判断「我能不能用这个框架」的**唯一依据**（77 行），而它此前**零守卫**且被 `usage-guide.test.ts` 的解析器**结构性**排除（那条规则只收「首列恰好是一个反引号标识符」的行，§7 首列是散文 ⇒ 整表零采集）。退化的样子：有人实现了出站注入 / 换了默认 client / 改了 store 幂等档位，表里那几行**变成假话而全绿** —— 比假守卫更隐蔽，**连断言都没有**。`gap` 的语义是「这句话今天为真、明天可能为假而没人会发现」（**不是功能缺失**），首版读数 **pin 38 / choice 9 / gap 30** |
| `packages/cli/test/native-pick.test.mjs` | 原生文件夹选择器的三平台分派（darwin `osascript` / win32 `powershell` / linux `zenity`→`kdialog`）、**三种取消形态**（darwin 认**错误码 `(-128)`** 而非本地化文案、win32 与 linux 落「两流皆空」）、输出归一化（尾斜杠 / 根目录 `/` 例外 / 空格 / 非 ASCII）、signal 视同取消，以及 `dev.ts` 的接线与 `/api/fs/pick` 路由在场 | 纯函数（平台可注入）+ 源码级接线断言；**绝不真调 `pickFolderNative()`** —— `osascript` / `zenity` 会弹**真对话框**把 CI 挂死。**反向验证（2026-09-26 补登记时实测 2 条）**：① linux 候选顺序对调（zenity 优先**是契约的一部分**）⇒ **恰好 1 条**（`linux → 先 zenity 后 kdialog；都没有 ⇒ null（调用方翻译成 501）`）；② darwin 取消判定去掉 `(-128)` 只留文案 ⇒ **2 条**（两条 darwin 取消用例）。复原后 sha256 逐字节一致。⚠️ 这条**此前没登记进 §1**（守卫在、登记缺）—— 与本仓记过的 `check-website-agent-readiness.mjs` 同一类：`guards-registry.test.ts` 只查「列出的路径在不在盘上」，查不出「盘上有守卫却没列」 | 非英文系统上「用户取消」被误判成失败（回 501，而不是静默降级到「浏览…」）；win32 / linux 的取消形态判错 ⇒ 真错误被当成取消，用户看到的是「点了什么都没发生」 |
