...
> ⚠️ **本节是治理开工时的快照**（2026-09-23，动手之前）—— 里面的行数、文件数、
> 「零守卫」「没钉这一条」都是**当时**的事实，**故意不改写**（本仓规矩：保留修订痕迹；
> 后来的改动在 §3 C 的补注与 §7 回填）。⚠️ 读到位置引用时注意：**路由表与 `STATIC`
> 白名单现在住在 `inspector-routes.ts`**（`inspector.ts` 818 → 413，见 §3 C 补注）。
