...
> **日期**：2026-09-11
>
> **后续（2026-09-22）**：本计划是 dev 环的**前半**（「看 trace」），已在 v0.8.x 落地。
> **后半**（「驱动 run」：prompt 控制台 / 能力选择 / 工作目录 / 多轮）见
> `docs/plans/2026-09-22-dev-debug-loop.md`，决策记录在 `docs/spec.md` §10 2026-09-22 ②。
> ⚠️ **本文档下面第 3 条架构里的 `NODE_OPTIONS=--import` 注入方式已被取代**（见 §非目标）——
> 它需要 Node ≥ 20.6 / ≥ 18.19 的版本闸，而 CLI 现在自己跑 `dev-runner.js` 子进程，
> 不再需要往 `NODE_OPTIONS` 里塞东西。本文档保留原样以留痕，**实现以 spec §10 为准**。
...
> **非目标的修订（2026-09-22，随后半落地）** —— 这三条要分开看，别当成「dev 环都不做」：
> - **run 重放执行**：**仍然不做**。后半做的是「**发一次新的** run」（面板 → `POST /run` → 真跑），
>   不是「把旧 run 照着原来的输入重放一遍」。两者相邻但不同：重放需要**持久化每次的输入**并
>   保证可重复（含随机性、时间、外部状态），那是另一个立项。想重放旧 run 用 trace diff / 分叉重放
>   （`agentia diff`，§6.4），与面板是两条路。
> - **live 流式逐 token 推送**：**仍然不做**（面板仍是 run 结束后可见）。
> - **历史归档检索**：**部分做了**，但**只在会话维度**：开了多轮的能力会有对话视图（`.agentia/session.json`），
>   面板靠 run 根的 `session.id` 把失败轮 join 回来。**跨 run 的检索 / 归档仍然不做**。
>
> 另外补一句**后半新增的**非目标：**不做能力直调**（绕过主 agent 直接调子 agent）——
> 用户已明确否决，这不是「暂时不做」而是设计前提（见 `2026-09-22-dev-debug-loop.md` §7）。
