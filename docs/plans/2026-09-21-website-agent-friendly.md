...
>
> 关联：`packages/website`（`404.astro` · `robots.txt.ts` · `sitemap.xml.ts` · `llms.txt.ts` · `layouts/Base.astro`）
> · `scripts/check-website-agent-readiness.mjs`（折在 `verify-all.sh` 第 8 步）
> · `.github/workflows/ci.yml` 的 `deploy-website`（新增「只报告」的打分步骤）
> · `AGENTS.md` 的 packages/website 段
>
...
### 3.2 URL 口径：`.html`（有意的，不是疏漏）

`sitemap.xml` 与 llms.txt 的站内链接**都用 `.html` 形态**。理由：站点自声明口径是 `.html`
（每页 og:url 由 `Base.astro` 按 `Astro.url.pathname` 算出 → `/docs.html`）、站内互链
（Nav / Footer / 正文）与 README、`package.json` 的 `homepage` 也全是 `.html`。

> 注：AFDocs 的 `llms-txt-coverage` 会把 `.html` 与干净路径**归一化后**再比对
> （`normalizeUrlPath` 里 `path.replace(/\.(?:md|mdx|html?)$/i, '')`），所以两种写法它都不介意 ——
> 但**人和索引器看到的是两份声明**，所以本仓统一到 `.html`，不在同一次改动里去动既有外链。
...
| `packages/website/src/pages/llms.txt.ts` | 「文档」节的链接改成由 `Astro.site` 拼的绝对地址（`abs()` 小工具） |
| `packages/website/src/layouts/Base.astro` | `<body>` 最前的 `<div class="llms-hint">`：`clip` / `clip-path:inset(50%)` 视觉隐藏、URL 写纯文本不套 `<a>`、必须在 `<nav>` 之前（三条约束各有实际后果，见文件内注释） |
| `scripts/check-website-agent-readiness.mjs` | 按**产物形状**核 6 类不变量（见 §5），折进 `verify-all.sh` 第 8 步 —— **不加第 9 步**（步骤数写在 CI 必需检查名里） |
...
`scripts/check-website-agent-readiness.mjs` 的 6 类断言：

1. `404.html` 存在（硬 404 的前提）
2. `robots.txt` 有绝对 `Sitemap:` 行
3. `sitemap.xml` 的 `<loc>` 集合 **==** dist 里实际的 `*.html` 集合（排除 404）
...
删 `404.html` / 链接改回相对 / 指引加 `display:none` / 指引挪到 `<nav>` 之后 /
`llms-full.txt` 改一个字节 / sitemap 少一个 `<loc>` / URL 套进 `<a>`，**7/7 全部咬住**。
