...
面向应用开发者的**声明式 agent 服务开发框架**：TS 装饰器 + DI + 模块；主 agent 作为路由器调度 tool/skill/subagent/prompt；**交付物是可上线的 Agent 服务而非对话助手**；**可观测是一等公民**——每次 run 产出一条 trace（调用树 + 每步 token/成本/耗时/错误类型），既是调试表面也是审计记录（见 §9）；运行时自研、参考 Claude 设计，底层调用 Messages API。
