...
/**
 * 能力 usage 聚合 —— `core/trace.ts` 从第一天就声明了「capability.usage = 其子孙 llm.turn 的聚合，
...
describe('TraceRecorder：capability span 的 usage = 子孙 llm.turn 聚合', () => {
  it('直接子 turn 的 usage 汇总到 capability span（结束时就地写入）', () => {
    const r = new TraceRecorder();
    const root = r.begin('run', 'agent.run', null);
    const capability = r.begin('capability', 'summarize', root);
    r.setAttribute(capability, 'skill', 'summarize');
    const t1 = r.begin('llm.turn', 'claude-opus-5', capability);
    r.end(t1, { usage: U(100, 20, 0.01) });
    const t2 = r.begin('llm.turn', 'claude-opus-5', capability);
    r.end(t2, { usage: U(50, 10, 0.005) });
    r.end(capability, { status: 'ok' });
    r.end(root, { status: 'ok' });

    const trace = r.snapshot('ok');
    const span = trace.spans.find((s) => s.spanId === capability)!;
    assert.deepEqual(
  ...
  it('嵌套 capability（skill 里跑 subagent）：外层聚合含内层，且不双算', () => {
    const r = new TraceRecorder();
    const root = r.begin('run', 'agent.run', null);
    const skill = r.begin('capability', 'pipeline', root);
    r.setAttribute(skill, 'skill', 'pipeline');
    // skill 自己先调一次模型
    const s1 = r.begin('llm.turn', 'claude-opus-5', skill);
    r.end(s1, { usage: U(10, 1, 0.001) });
    // 再起一个 subagent（它是 skill 的子 capability）
    const sub = r.begin('capability', 'researcher', skill);
  ...
  it('capability 内一次模型都没调 → usage 保持 undefined（不造假 0）', () => {
    const r = new TraceRecorder();
    const root = r.begin('run', 'agent.run', null);
    const capability = r.begin('capability', 'noop', root);
    r.setAttribute(capability, 'skill', 'noop');
    r.end(capability, { status: 'ok' });
    r.end(root, { status: 'ok' });
    const span = r.snapshot('ok').spans.find((s) => s.spanId === capability)!;
  ...
  it('调用方显式给 usage 时以显式值为准（不被聚合覆盖）', () => {
    const r = new TraceRecorder();
    const root = r.begin('run', 'agent.run', null);
    const capability = r.begin('capability', 'x', root);
    const t = r.begin('llm.turn', 'm', capability);
  ...
  it('未定价（无 costEstimate）的子孙不产出 costEstimate 字段', () => {
    ...
    const root = r.begin('run', 'agent.run', null);
    const capability = r.begin('capability', 'x', root);
    const t = r.begin('llm.turn', 'mystery-model', capability);
    r.end(t, { usage: U(5, 5) });
    r.end(capability, { status: 'ok' });
    r.end(root, { status: 'ok' });
    const span = r.snapshot('ok').spans.find((s) => s.spanId === capability)!;
