...
/** 从 add 参数解析真实包名：支持 name[@version]、@scope/name[@version]、本地路径（file:） */
function resolvePackageName(arg: string): string {
  if (arg.startsWith('.') || arg.startsWith('/')) {
    const pkgFile = join(resolve(arg), 'package.json');
    if (!existsSync(pkgFile)) {
      throw new Error(`本地包 ${arg} 缺少 package.json`);
    }
    const name = (JSON.parse(readFileSync(pkgFile, 'utf8')) as { name?: string }).name;
    if (typeof name !== 'string' || name.length === 0) {
      throw new Error(`本地包 ${arg} 的 package.json 缺少 name 字段`);
    }
    return name;
  }
